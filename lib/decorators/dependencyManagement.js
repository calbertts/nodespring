...
function Inject(typeToInject) {
  ...
  return function (target, property, descriptor) {
    ...
    _moduleContainer.ModuleContainer.addDependency(targetName, property, typeToInject.name);
  };
}
...
function Interface(interfaceBase) {
  ...
  interfaceClass.prototype.constructor = new Function(interfaceClass.name, " return function " + interfaceClass.name + "(){ " + "throw TypeError('NodeSpring Error: Cannot construct " + interfaceClass.name + " instances directly, because it is an Interface')}")();
